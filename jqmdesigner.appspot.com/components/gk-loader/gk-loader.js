+(function (window, requirejs, $) {
  ...
  function dirname(url, level) {
    return url.split('?')[0].split('index.html').slice(0, level || -1).join('index.html');
  }
  ...
  function normalize(path) {
    var parts = (path = addProtocol(path)).split('://'),
      ...
      p;
    if (parts.length > 1) {
      host = parts[0] + '://' + parts[1].split('index.html')[0];
    ...
    return host + result.join('index.html');
  }
